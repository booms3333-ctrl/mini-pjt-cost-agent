...
- **이걸로 뭐가 좋아지는가**: 자연어 질의 한 번으로 3개 클라우드 비용을 통합 조회하고, 일 단위 이상탐지로 발생 다음 날 인지 → 조사 시간 단축 + 손실 누적 방지. 대화가 이어지므로 이전에 조회한 리소스·수치를 매번 다시 안 물어봐도 된다
...
**대화 이어가기**: `AsyncSqliteSaver`가 `thread_id`별로 대화 상태를 `data/checkpoints.db`에
영구 저장한다 — 같은 `thread_id`로 다시 질문하면 이전 대화 맥락(조회했던 리소스 ID,
언급된 수치, 승인·실행 여부 등)을 이어서 답한다. 서브 에이전트 시스템 프롬프트도
바로 앞의 질문·답변을 항상 참고하도록 지시돼 있어("이전에 무엇을 물었는지 다시
물으면 그대로 알려주세요"), "그거 얼마였지?"처럼 이전 답을 다시 묻는 후속 질문도
새로 조회하지 않고 답할 수 있다. 메시지가 20개(`HISTORY_SUMMARY_THRESHOLD`)를 넘으면
오래된 부분을 요약으로 교체하고 최근 6개(`HISTORY_KEEP_RECENT`)만 원문으로 남겨서,
대화가 길어져도 체크포인트가 무한정 커지지 않게 관리한다(`pipeline.py`).
